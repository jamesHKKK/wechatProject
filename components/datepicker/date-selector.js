Component({
  ...
  data: {
    ...
    dateIndex: [1, 1, 1],
    date: [], // 时间数据
    recentTime: [0,0,0], // 存储选中时间 对应 properties-selectTime
    // endTime:[],
  ...
  methods: {
     ...
     */
    btn_time(beginTime,endTime,defSelectTime,selectTime,type,dateIndex){
      this.data.beginTime = beginTime;
      this.data.endTime = endTime;
      this.data.defSelectTime = defSelectTime;
      this.data.selectTime = selectTime;
      this.data.type = type;
      this.data.recentTime = selectTime;
      this.data.dateIndex = dateIndex;
      this.init()
      let _data = this.makeDate(this.data.recentTime)
      this.setData({
        date: _data.date,
    ...
    makeDate: function(recentTime) {
      ...
      this.loop(0, beginTime.length, function(i) {
        var min = 0,
          max = 0,
          cur = 0;
          if(type=="yyyy-MM-dd hh:mm"||type=="yyyy-MM-dd hh:mm至yyyy-MM-dd hh:mm"){
            switch (i) {
              case 0:
          ...
          if(type=="yyyy-MM-dd hh:mm:ss"||type=="yyyy-MM-dd hh:mm:ss至yyyy-MM-dd hh:mm:ss"){
            switch (i) {
              case 0:
                arr.push(that.buildArr(beginTime[i], endTime[i], "年"));
                dateIndex[i] = recentTime[i] - beginTime[i];
          ...
          if(type=="hh:mm:ss"||type=="hh:mm:ss至hh:mm:ss"){
            switch (i) {
              
              case 0:
                min = that.checkTimeArr(beginTime, recentTime, 0) ? beginTime[i] : 0;
                max = that.checkTimeArr(endTime, recentTime, 0) ? endTime[i] : 23;
                dateIndex[i] = that.checkTimeArr(beginTime, recentTime, 0) ? recentTime[i] - beginTime[i] : recentTime[i];
                arr.push(that.buildArr(min, max, "时"))
                break;
              case 1:
                min = that.checkTimeArr(beginTime, recentTime, 1) ? beginTime[i] : 0;
                max = that.checkTimeArr(endTime, recentTime, 1) ? endTime[i] : 59;
                // console.log(min + ":" + max);
                dateIndex[i] = that.checkTimeArr(beginTime, recentTime, 1) ? recentTime[i] - beginTime[i] : recentTime[i];
                arr.push(that.buildArr(min, max, "分"))
                break;
              case 2:
                min = that.checkTimeArr(beginTime, recentTime, 2) ? beginTime[i] : 0;
                max = that.checkTimeArr(endTime, recentTime, 2) ? endTime[i] : 59;
                // console.log(min + ":" + max);
                dateIndex[i] = that.checkTimeArr(beginTime, recentTime, 2) ? recentTime[i] - beginTime[i] : recentTime[i];
                arr.push(that.buildArr(min, max, "秒"))
                break;
            }
          }
    ...
    init: function() {
      ...
      if (typeof this.data.selectTime == 'string') { //占位符
        ...
        let cantant = this.data.selectTime==null?[]:this.data.selectTime
        _recentTime = cantant.concat()
